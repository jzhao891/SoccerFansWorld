...
export default function CardFramePage() {
  ...
  function handlePhoto(event: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => { setPhotoUrl(String(reader.result)); resetPreview(); };
  ...
  async function callRender(tier: RenderTier): Promise<string> {
    const res = await fetch("/api/frames/render", {
      ...
      body: JSON.stringify({ templateId: frame.id, photoDataUrl: photoUrl, selection, inputs, tier }),
    });
          ...
          <div className="rounded-[2rem] border border-white/15 bg-slate-950/55 p-5 shadow-2xl backdrop-blur sm:p-7">
            ...
            <div className="space-y-6">
                    ...
                    {assetsForDimension(dimension).map((asset) => {
                      ...
                      return (
                        <button key={asset.id} type="button"
                          onClick={() => { setSelection((prev) => ({ ...prev, [dimension]: active && !required ? undefined : asset.value })); resetPreview(); }}
                          className={`relative overflow-hidden rounded-2xl border-2 transition ${
                            active ? "border-emerald-300 shadow-[0_0_16px_rgba(52,211,153,0.4)]" : "border-white/15 hover:border-white/35"
                          }`}>
                          <img src={asset.thumbnail ?? asset.src} alt={asset.label} className="h-20 w-full object-cover" />
                          <div className="absolute inset-x-0 bottom-0 bg-slate-950/70 py-1 text-center text-[0.55rem] font-black uppercase tracking-wide text-white">{asset.label}</div>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="grid gap-4">
                  {frame.inputs.map((field) => (
                    <label key={field.id} className="block">
                      ...
                      <input value={inputs[field.id] ?? ""} placeholder={field.placeholder} maxLength={field.maxLength}
                        onChange={(e) => { setInputs((prev) => ({ ...prev, [field.id]: e.target.value })); resetPreview(); }}
              ...
              {!canRender && photoUrl && missingRequired.length > 0 && (
                <p className="text-center text-xs text-amber-300">Pick a {missingRequired.join(", ")} to continue.</p>
              )}
            </div>
          </div>
